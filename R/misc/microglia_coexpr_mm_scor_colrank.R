...
# Save out
out_dir <- "/space/scratch/amorin/TR_singlecell/Microglia/Mm_scor"
dir.create(out_dir, showWarnings = FALSE)
aggr_path <- file.path(out_dir, "aggregate_matrix_colrank.tsv")
...
if (!file.exists(aggr_path) || !all_ids_have_cor) {
  ...
  aggr_l <- aggr_celltype_coexpr_colrank(ct_df = ct_df,
                                         pc_df = pc_df,
                                         cor_method = "spearman",
                                         ncores = ncore,
                                         in_dir = amat_dir,
