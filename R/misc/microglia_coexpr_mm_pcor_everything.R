...
# Save out
out_dir <- "/space/scratch/amorin/TR_singlecell/Microglia/Mm_pcor_everything"
dir.create(out_dir, showWarnings = FALSE)
aggr_path <- file.path(out_dir, "aggregate_matrix.tsv")
...
if (!file.exists(aggr_path) || !all_ids_have_cor) {
  ...
  aggr_l <- aggr_celltype_coexpr(ct_df = ct_df, 
                                 pc_df = pc_df,
                                 cor_method = "pearson",
                                 ncores = ncore,
                                 in_dir = amat_dir,
